...
export default function ErrandGoGetStartedPage() {
  ...
  const [pendingAction, setPendingAction] = useState<PostLoginAction>(null);
  const [showLoginPrompt, setShowLoginPrompt] = useState(false);
  const [toast, setToast] = useState("");

  const activeMethodLabel = useMemo(() => {
    if (auth.method === "wallet") return "Wallet connected";
    if (auth.method === "google") return "Google connected";
    if (auth.method === "email") return "Email connected";
    return "Guest mode";
  }, [auth.method]);

  const completeAction = (action: Exclude<PostLoginAction, null>) => {
    const messages: Record<Exclude<PostLoginAction, null>, string> = {
      "create-escrow":
        "Escrow flow opened. You can now proceed to create a protected transaction.",
      "start-trade":
        "Trade flow opened. The action is now available in the workspace.",
      "create-milestone":
        "Milestone flow opened. You can now structure phased releases.",
      "request-payment":
        "Payment request flow opened. The action is ready to continue.",
    };

    setToast(messages[action]);
  };

  const handleProtectedAction = (action: Exclude<PostLoginAction, null>) => {
  ...
  const handleSignOut = () => {
    ...
    setPendingAction(null);
    setToast(
      "You have been signed out. Protected actions now require login again."
    );
  };
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-slate-500">Payments & Services</div>
            </div>
          </div>
          ...
          <Surface className="overflow-hidden border-slate-200 bg-[radial-gradient(circle_at_top_left,#ffffff,#f8fafc_55%,#eef2ff)]">
            <CardContent className="p-6 md:p-8">
              ...
              <h1 className="mt-4 max-w-3xl text-3xl font-semibold tracking-tight text-slate-950 md:text-5xl md:leading-tight">
                Get started with ErrandGo using the same premium workspace feel.
              </h1>

              <p className="mt-4 max-w-2xl text-base leading-8 text-slate-600 md:text-lg">
                Sign in with wallet, Google, or email, then unlock escrow,
                trades, milestone plans, and payment actions from one clean
                operating view.
              </p>

              <div className="mt-6 flex flex-wrap gap-2.5">
                <FeaturePill>Escrow-ready</FeaturePill>
                <FeaturePill>P2P trade flows</FeaturePill>
                <FeaturePill>Milestone releases</FeaturePill>
                <FeaturePill>Protected payment actions</FeaturePill>
              </div>
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                ...
                <MiniStat
                  label="Method"
                  value={auth.method ? activeMethodLabel : "Choose one"}
                />
              </div>

              <div className="mt-8 rounded-[28px] border border-slate-200 bg-white p-5 md:p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                      Any protected action opens a login prompt when the user is
                      not authenticated. Once signed in, the same action
                      continues automatically to simulate a wired flow.
                    </p>
                  </div>
          ...
          <Surface>
            <CardContent className="p-6 md:p-8">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-600">
                    Start with a preferred sign-in method, then continue
              ...
              <div className="mt-6 grid gap-3">
                <Button
                  variant="outline"
                  className="h-12 justify-start rounded-2xl border-slate-200 bg-slate-50 text-slate-900 hover:bg-white"
                  onClick={() => {
                    setPendingAction(null);
                    setShowLoginPrompt(true);
                  }}
                ...
                >
                  <Mail className="mr-3 h-4.5 w-4.5" />
                  Continue with email
                </Button>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {quickActions.map((item) => (
                  <ActionTile
                    ...
                    onClick={() => handleProtectedAction(item.id)}
                  />
                ))}
              </div>
              ...
              {auth.isAuthenticated ? (
                ...
                <div className="mt-6 rounded-[24px] border border-amber-200 bg-amber-50 p-4">
                  ...
                  <div className="mt-1 text-sm leading-6 text-amber-700">
                    Protected actions stay locked until the user authenticates.
                  </div>
                </div>
              )}
            </CardContent>
